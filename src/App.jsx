...
import { auth as firebaseAuth, storage as firebaseStorage, db as firestoreDb, googleProvider } from './config/firebase';
import {
...
function App() {
  ...
  const [authForm, setAuthForm] = useState({ name: '', email: '', password: '', rememberMe: true });
  ...
  const handleAuth = async (e) => {
    ...
    try {
      ...
      await setPersistence(firebaseAuth, authForm.rememberMe ? browserLocalPersistence : browserSessionPersistence);
      if (isLoginView) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (!userSnap.exists()) {
        const role = fbUser.email === 'tilakmishra.76@gmail.com' ? 'admin' : 'student';
      ...
      const msg = err.code === 'auth/popup-closed-by-user' ? 'Sign-in cancelled.' : err.message;
      setAuthError(msg);
      showToast(msg, 'error');
    }
    setLoading(false);
      ...
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-primary)', padding: '1rem' }}>
        <div className="panel" style={{ padding: '3rem', position: 'relative', background: 'var(--bg-primary)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '32px', overflow: 'hidden', width: '100%', maxWidth: '420px', boxShadow: '0 20px 40px rgba(0,0,0,0.4)' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
            ...
            <h2 style={{ fontSize: '2rem', fontWeight: '900', color: 'white', letterSpacing: '-0.02em' }}>{isLoginView ? 'Welcome Back' : 'Create Account'}</h2>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginTop: '8px' }}>Exam Cracker Coaching Centre</p>
          </div>
          ...
          <form onSubmit={async (e) => { e.preventDefault(); await handleAuth(e); }} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            {!isLoginView && (
              <div>
            ...
            {isLoginView && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <input type="checkbox" id="rememberMe" checked={authForm.rememberMe} onChange={e => setAuthForm({ ...authForm, rememberMe: e.target.checked })} style={{ width: '16px', height: '16px', accentColor: '#38bdf8', cursor: 'pointer' }} />
                <label htmlFor="rememberMe" style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', cursor: 'pointer', userSelect: 'none' }}>Remember me</label>
              </div>
            )}

            <button type="submit" className="btn btn-primary" style={{ padding: '1.1rem', borderRadius: '16px', fontWeight: '800', fontSize: '1rem', marginTop: '0.25rem' }} disabled={loading}>
              {loading ? 'Please wait...' : (isLoginView ? 'Sign In' : 'Register')}
            </button>
          </form>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', margin: '1.5rem 0' }}>
            ...
            <span style={{ color: '#64748b', fontSize: '0.8rem', fontWeight: '600', whiteSpace: 'nowrap' }}>or continue with</span>
            <div style={{ flex: 1, height: '1px', background: 'var(--border-glass)' }} />
          </div>
          ...
          <button
            ...
            disabled={loading}
            style={{ width: '100%', padding: '0.9rem 1.25rem', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.03)', color: 'white', fontWeight: '700', fontSize: '0.95rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', transition: 'all 0.2s' }}
          >
              ...
              <div style={{ width: '100%', maxWidth: '520px' }}>
                ...
                <div style={{ background: 'rgba(56,189,248,0.04)', border: '1px solid rgba(56,189,248,0.1)', borderRadius: '14px', padding: '1rem 1.25rem', marginBottom: '1.5rem', display: 'flex', gap: '12px', alignItems: 'center' }}>
                  ...
                  <p style={{ margin: 0, fontSize: '0.82rem', color: '#94a3b8', lineHeight: '1.5' }}>
                    Need urgent access? Contact <strong style={{ color: '#f8fafc' }}>Yogesh Sir</strong> or <strong style={{ color: '#f8fafc' }}>Tilak Mishra Sir</strong> directly.
                  </p>
                </div>
